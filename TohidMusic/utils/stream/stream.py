...
import config
from TohidMusic import Carbon, YouTube, app
from TohidMusic.core.call import Tohid
from TohidMusic.misc import db
...
from TohidMusic.utils.exceptions import AssistantErr
from TohidMusic.utils.inline.play import stream_markup, queue_markup, telegram_markup
from TohidMusic.utils.inline.playlist import close_markup
from TohidMusic.utils.pastebin import Tohidbin
from TohidMusic.utils.stream.queue import put_queue, put_queue_index
from TohidMusic.utils.thumbnails import gen_thumb, gen_qthumb
...
async def stream(
    ...
    if forceplay:
        await Tohid.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Tohid.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Tohid.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
